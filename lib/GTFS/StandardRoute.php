<?php
	...
	class StandardRoute implements RouteInterface {
		...
		public function __construct($route_id = false, $Provider = false) {
			...
			if (filter_var($route_id, FILTER_VALIDATE_INT)) {
				$query = sprintf("SELECT route_id, route_short_name, route_long_name, route_desc, route_type, route_url, route_color, route_text_color FROM %s_routes WHERE id = %s", $this->Provider->getDbPrefix(), $route_id);
		...
		public function getPath() {
			...
			if (!$path = getMemcacheObject($mckey)) {
				...
				if (isset($trip)) {
					...
					setMemcacheObject($mckey, $path, strtotime("+1 month"));
				}
			}
